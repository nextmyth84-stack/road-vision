...
def gpt_extract(img_bytes, want_early=False, want_late=False, want_excluded=False):
    ...
    user = (
        ...
        "\n"
        "1) 'PC학과', '1종 기능장', '2종 기능장', '기능장 초소', '도주전산', '기능시험', '교육장' 등은 제외합니다.\n"
        "2) ‘교육’, ‘출장’, ‘휴가’, ‘조퇴’, ‘외출’, ‘연차’, ‘연가’, ‘공가’, ‘돌봄’이 표시된 인원은 제외자로 추출합니다.\n"
        "3) 이름 옆 괄호(A-합, B-불 등)는 코스 및 결과로 분리합니다.\n"
        "   - 예: ‘김성연(A-합)’ → {name:'김성연', course:'A코스', result:'합격'}\n"
        "4) 오전/오후 표기 구간 중 이미지에 표시된 시간대만 인식합니다.\n"
        "5) ‘10시 출근’, ‘10.5시’, ‘외출 후 복귀’ 등은 late_start 로, ‘조퇴’, ‘14.5시 퇴근’ 등은 early_leave 로 추출합니다.\n"
        "6) 최종 결과는 JSON으로 반환합니다.\n"
        "\n"
        "JSON 예시:\n"
        "{\n"
        "  \"names\": [\"김성연(B합)\",\"김병욱(A불)\"],\n"
