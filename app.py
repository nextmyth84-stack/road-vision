...
########################################################################
try:
    # [수정] st.secrets["general"]["GOOGLE_APPLICATION_CREDENTIALS"] -> st.secrets["GOOGLE_APPLICATION_CREDENTIALS"]
    # secrets.toml 구조에 따라 [general]이 없을 수도 있습니다.
    # 만약 [general] 섹션 하위에 키를 두셨다면 원본 코드를 사용하세요.
    if "general" in st.secrets:
        cred_data = json.loads(st.secrets["general"]["GOOGLE_APPLICATION_CREDENTIALS"])
    else:
        cred_data = json.loads(st.secrets["GOOGLE_APPLICATION_CREDENTIALS"])
...
########################################################################
# 3) [수정됨] Vision API OCR 함수 (검색 영역 확장 + 이름 필터링)
########################################################################
...
def extract_doro_juhaeng_workers(file_content):
    ...
    이미지에서 '도로주행' 근무자 목록을 추출합니다. (검색 영역 수정됨)
    """
    if not file_content:
    ...
    try:
        ...
        # 2️⃣ 이름이 위치할 예상 영역 계산
        doro_y_end = doro_box.vertices[3].y
        
        # [수정 1] doro_y_limit: '도로주행' 아래 600px -> 1500px까지 확장
        # 이름 목록이 길어져도 다 포함하도록 넉넉하게 수정
        doro_y_limit = doro_y_end + 1500 

        name_col_x_start = name_header_box.vertices[0].x - 30
        
        # [수정 2] name_col_x_end: '성명' 오른쪽 150px -> 250px까지 확장
        # 이름이 컬럼에서 약간 벗어나도 포함하도록 넉넉하게 수정
        name_col_x_end = name_header_box.vertices[1].x + 250

        workers = []
        seen_workers = set() # 중복 제거용
        ...
        for block in page.blocks:
            for paragraph in block.paragraphs:
                ...
                if is_in_doro_rows and is_in_name_column:
                    ...
                    # [수정 3] v2의 이름 필터링(정규식) 원리 적용
                    name = para_text.strip().replace(" ", "") # 공백 제거
                    
                    if not name:
                        continue
                    
                    # '성명' 혹은 '성명'과 유사한 단어 제외
                    if fuzz.ratio(name, "성명") > threshold:
                        continue
                        
                    # 2~5글자의 한글 이름인지 확인 (v2 로직)
                    if re.fullmatch(r"[가-힣]{2,5}", name):
                        if name not in seen_workers:
                            workers.append(name)
                            seen_workers.add(name)

        # [수정 4] OCR 인식 순서가 아닌, 좌표 순서대로 정렬 (선택사항)
        # workers 리스트는 이미 y좌표 순서대로 append될 확률이 높지만,
        # document_text_detection은 블록/문단 순서가 복잡할 수 있으므로
        # 여기서는 중복 제거 후 리스트를 반환합니다. (정렬 로직은 제거함)
        
        # workers = sorted(set(workers)) # -> 순서 유지를 위해 set만 사용
...
########################################################################
# 4) 유틸리티 함수: 순번 계산, JSON 로드 (원본과 동일)
########################################################################
...
########################################################################
# 5) 사용자 입력: 전일 근무자, 정비차량 등 (사이드바 - 원본과 동일)
########################################################################
...
if 'morning_workers' in st.session_state and 'afternoon_workers' in st.session_state:
    ...
    if st.button("② 최종 근무 배정 생성", type="primary", key="generate_assignment_button"):
        with st.spinner("배정 로직을 계산 중입니다..."):
            ...
            # [버그 수정] afternoon_lines 생성 로직이 누락되어 있었습니다.
            # 오후 결과 텍스트 생성
            afternoon_lines = []
            ...
            with res_col2:
                # [버그 수정] 이 부분이 정상 작동하도록 위에서 afternoon_lines를 생성했습니다.
