# app.py — 도로주행 근무자동배정 v7.15.8 (완전본)
...
EMP_FILE = "근무자명단.json"
KEY_FILE = "열쇠순번.json"
GYOYANG_FILE = "교양순번.json"
SUDONG_FILE = "수동순번.json"
...
def correct_name(name, valid_list):
    """OCR 교정 — 근무자명단과 유사도 비교"""
    n = normalize_name(name)
    if not valid_list: return n
    best = difflib.get_close_matches(n, [normalize_name(x) for x in valid_list], n=1, cutoff=0.7)
...
def clipboard_copy_button(label, text):
    ...
    <button onclick="navigator.clipboard.writeText(`{text}`)" 
            style="background:#4CAF50;color:white;padding:6px 14px;border:none;border-radius:6px;cursor:pointer;">
            {label}
    </button>
    """
...
if not os.path.exists(GYOYANG_FILE):  save_json(GYOYANG_FILE, default_gy)
if not os.path.exists(SUDONG_FILE):   save_json(SUDONG_FILE, default_sd)
...
if not os.path.exists(EMP_FILE):      save_json(EMP_FILE, default_emp)
...
gyoyang_order = load_json(GYOYANG_FILE, default_gy)
sudong_order  = load_json(SUDONG_FILE, default_sd)
...
with st.sidebar.expander("📘 교양 순번", expanded=False):
    ...
    if st.button("💾 교양순번 저장"):
        save_json(GYOYANG_FILE, _list(gy_text)); st.sidebar.success("저장 완료"); st.rerun()

with st.sidebar.expander("🔧 1종 수동 순번", expanded=False):
    sd_text = st.text_area("1종 수동 순번", "\n".join(sudong_order), height=150, key="sd_text")
    sudong_count = st.radio("1종 수동 인원수", [1, 2], index=0, horizontal=True, key="sudong_count")
    if st.button("💾 1종 순번 저장"):
        save_json(SUDONG_FILE, _list(sd_text)); st.sidebar.success("저장 완료"); st.rerun()
...
# =====================================
# GPT OCR (근무자 + 근무제외 + 코스점검 A/B 합불)
# =====================================
def gpt_extract(img_bytes, detect_excluded=True):
    """
    반환:
      names: 괄호 제거 + 근무자명단 기반 교정
      course_info: [{"name":이름, "course":"A코스 합격/불합격" | "B코스 합격/불합격"}]
      excluded: 근무제외자 목록(교정 반영)
    """
    b64 = base64.b64encode(img_bytes).decode()
    user = (
        "이 이미지는 운전면허시험 근무표입니다.\n"
        "1) 도로주행 근무자 이름을 JSON으로 추출하세요.\n"
        "2) 이름 옆 괄호는 포함하여 반환하세요. 예: (A-합), (A합), (B-불) 등\n"
        "3) '지원','인턴','연수' 포함자는 제외하세요.\n"
    )
    if detect_excluded:
        user += "4) 상단의 '휴가','교육','출장','공가','연가','연차','돌봄' 줄의 이름을 'excluded' 배열로 반환하세요.\n"
    ...
    try:
        ...
        for n in full:
            # 괄호 내용에서 A/B만 기준으로 코스, '합/불'로 결과 판정
            m = re.search(r"\(([A-Za-z가-힣\- ]+)\)", n)
            pure = re.sub(r"\(.*?\)", "", n).strip()
            names_raw.append(pure)

            if m:
                text = m.group(1).replace("-", "").replace(" ", "").upper()  # 특수문자 제거
                ...
                if course_type and result_type:
                    course_info.append({"name": pure, "course": f"{course_type}코스 {result_type}"})

        # 오타 교정 (근무자명단 기반)
        valid = employees
        def correct_to_original(x):
            n = normalize_name(x)
            # 정규화→원본 매핑
            mapping = {normalize_name(v): v for v in valid}
            if n in mapping: return mapping[n]
            cand = difflib.get_close_matches(n, list(mapping.keys()), n=1, cutoff=0.70)
            return mapping[cand[0]] if cand else x

        names = [correct_to_original(x) for x in names_raw]

        excluded = js.get("excluded", []) if detect_excluded else []
        excluded = [correct_to_original(x) for x in excluded]
...
def afternoon_assign():
    ...
    new_joiners = [nm for nm in afternoon_drivers if normalize_name(nm) not in morning_norms]
    missing     = [nm for nm in morning_drivers  if normalize_name(nm) not in afternoon_norms]

    if new_joiners: lines.append(" • 신규 투입 인원: " + ", ".join(sorted(new_joiners)))
    if missing:     lines.append(" • 빠진 인원: " + ", ".join(sorted(missing)))
    if not new_joiners and not missing: lines.append(" • 변동 없음")

    # === 미배정 차량 (오전엔 있었는데 오후에 없는 것만)
