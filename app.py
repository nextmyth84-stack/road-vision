...
def gpt_extract_names_from_image(image_bytes, hint="도로주행 근무자"):
    ...
    # [수정됨] 프롬프트: 모델이 '모든' 이름을 리스트로 반환하도록 명확하게 지시
    system = (
        "당신은 이미지 속 표에서 특정 섹션의 '사람 이름 목록'을 찾아 추출하는 도구입니다. "
        "결과는 반드시 JSON 형식으로만 반환해야 하며, 다른 설명은 절대 추가하지 마십시오."
    )
    user = (
        f"이미지에 '도로주행' 또는 '{hint}'와 관련된 근무자 명단이 있습니다. "
        f"해당 섹션에 있는 *모든* 사람의 이름을 순서대로 추출하세요.\n"
        "이름 외에 괄호, 메모 (예: A-합, B-불), 숫자, 영문 등은 모두 제거하세요.\n"
        "이름은 한글 2~5자만 유효합니다.\n"
