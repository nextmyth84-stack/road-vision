...
def gpt_extract_names_from_image(image_bytes, hint="도로주행 근무자"):
    ...
    # [수정됨] 프롬프트: SyntaxError 수정 (JSON 예시 부분의 따옴표 변경)
    system = (
        "당신은 이미지 속 표(테이블)에서 특정 조건에 맞는 사람 이름 목록을 찾아 추출하는 도구입니다. "
        "결과는 반드시 JSON 형식({'names': [...]})으로만 반환해야 하며, 다른 설명은 절대 추가하지 마십시오."
    )
    user = (
        f"이미지 속 표(테이블)에서 '{hint}' (예: '오전 도로주행')에 해당하는 근무자 이름만 추출하세요.\n"
        "정확한 추출 규칙은 다음과 같습니다:\n"
        "1. 표의 여러 행(row) 중에서, **맨 왼쪽 첫 번째 칸(열)에 '도로주행'이라고 적힌 행**만 찾으세요.\n"
        "2. 그 '도로주행' 행의 두 번째 칸(열)에는 '오전' 또는 '오후'가 표시되어 있습니다.\n"
        "3. 그 '도로주행' 행의 세 번째 칸(열)에는 '숫자'가 있습니다.\n"
        "4. 그 '도로주행' 행의 **네 번째 칸(열)에 실제 근무자의 '이름'**이 적혀 있습니다.\n"
        f"5. '{hint}' 조건에 맞는 '도로주행' 행을 모두 찾아, 각 행의 **네 번째 칸(열)에 있는 '이름'만** 순서대로 모두 추출하세요.\n"
        "\n"
