...
st.title("🚗 도로주행 근무자 자동배정 (GPT-4o-mini OCR 버전)")

# 모델 이름 (이미지 입력 가능한 모델로 설정하세요)
GPT_IMAGE_MODEL = st.sidebar.text_input("GPT 이미지 모델 (예: gpt-4o-mini)", value="gpt-4o-mini")
...
def analyze_image(image_bytes):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "너는 OCR 전문가야. 이미지 속 도로주행 근무자 이름을 정확히 추출해."
            },
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "이 이미지에서 근무자 이름만 줄바꿈으로 구분해 추출해줘."},
                    {
                        "type": "image_url",
                        "image_url": {
